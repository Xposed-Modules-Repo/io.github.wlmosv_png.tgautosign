...
### 2026-09-08 · 插件版修复记录（LSPilot jmb界面版 v1.1.0）

- 恢复 sendRequest 双触发去重（重复学习 / 重复标记已签）
- 修复启动补签被节流窗口误杀：启动 10 秒后的补签改为强制执行，此前会被网络活动触发的例行检查占用节流窗口而静默跳过
- 补签增加摘要日志（目标/已签/处理），节流跳过也留痕
- 运行日志追加写入插件目录 runtime.log
- 修复管理菜单点击错位（BSH 多 lambda 捕获绑定错误导致点击任意项都进设置），改标签分发
- 修复菜单创建时 Telegram 闪退（颜色字面量被 BSH 当作 long）
- 菜单配色适配深浅色主题
- 管理对话框宿主改为主 Activity（原判断 ChatActivity 非 Activity，菜单无法弹出）
- 目标列表/添加改为视图内按钮，规避对话框正按钮兼容问题
