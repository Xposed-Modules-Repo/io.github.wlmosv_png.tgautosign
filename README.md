...
- **回调可控化（v1.4.0）**：➕「🔘 捕获」列出该消息所有内联按钮、点选可绑多个（不受关键词限制）；每条目标自带「前置命令序列」（如 /start）先拉面板再重放；🔬 调试台 / 🧪 测试 即时发一次并显示机器人返回；🔁 一键把误判成指令的旧回调重绑
...
### v1.4.0（2026-09-14）—— 大改版：回调可控 + 全新界面 + 配置彻底可清理

- ➕ 回调「捕获 / 绑定 / 调试台 / 测试 / 前置命令 / 重绑」全套；自动学习与关键词解耦，默认不再误加
- 🔧 删除配置跨全部账号彻底清除 + 二次确认 + 🧹 清空所有配置；修复回调重启变指令
- 🎨 卡片化界面与概览状态卡、日志分级着色、使用教程、自诊断
