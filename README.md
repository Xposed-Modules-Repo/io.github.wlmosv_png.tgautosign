...
- 🐛 **修复**：自动签到成功后日历不绿（漏记签到记录，已回填历史）；子界面层层叠新窗口、退出要按很多次、光标卡死
- 🎨 **界面重排**：主菜单分 5 组（核心/工具/数据/系统/维护）；目标列表三列对齐 + 状态色区分；新增「未签置顶 / 按名称」排序
- ✨ **标题动效 4 连**：霓虹呼吸 / 逐字波浪 / RGB 流光 / 键盘敲击，每次打开轮换，深浅色双主题契合
- 📣 **加入群组**：主菜单「系统」组新增入口，一键跳 TG 交流群
- 🔌 宿主新增 NextAlone Nagram；Nagram XF 30dcd6c 构建注入会导致启动无响应，已自动跳过
